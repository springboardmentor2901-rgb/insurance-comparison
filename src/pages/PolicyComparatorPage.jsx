...
import { policies, policyTypes } from '../data/mockData';

export default function PolicyComparatorPage() {
    const [selectedPolicies, setSelectedPolicies] = useState([]);
    const [viewMode, setViewMode] = useState('select'); // 'select' or 'compare'
            ...
            <div className="page">
                <div className="container">
                    ...
                    <div style={{ marginBottom: '24px', display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
                        <button className="btn btn-secondary" onClick={() => setViewMode('select')}>
                            ← Back to Selection
                        </button>
                        <button className="btn btn-outline btn-sm" onClick={() => { setSelectedPolicies([]); setViewMode('select'); }}>
                            Clear All & Start Over
                        </button>
                    </div>

                    {/* Selected Policy Summary Cards */}
                        ...
                        {selectedPolicyData.map(p => (
                            <div key={p.id} className="glass-card" style={{ padding: '20px', textAlign: 'center', borderColor: 'var(--border-active)' }}>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        const remaining = selectedPolicies.filter(id => id !== p.id);
                                        setSelectedPolicies(remaining);
                                        if (remaining.length < 2) setViewMode('select');
                                    }}
                                >
                                    ✕ Remove
                                </button>
                        ...
                        <table className="comparison-table">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id}>
                                            <span className={`badge ${typeColors[p.type]}`}>{p.type}</span>
                                        </td>
                                    ))}
                                </tr>
                                <tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id} className={getBestValue('coverage') === p.id ? 'best' : ''}>
                                            {formatCurrency(p.coverage)}
                                            {getBestValue('coverage') === p.id && ' ✓'}
                                        </td>
                                    ))}
                                </tr>
                                <tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id} className={getBestValue('premium') === p.id ? 'best' : ''}>
                                            {formatCurrency(p.premium)}
                                            {getBestValue('premium') === p.id && ' ✓'}
                                        </td>
                                    ))}
                                </tr>
                                <tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id}>
                                            {formatCurrency(Math.round(p.premium / 12))}
                                        </td>
                                    ))}
                                </tr>
                                <tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id} className={getBestValue('duration') === p.id ? 'best' : ''}>
                                            {p.duration} {p.duration === 1 ? 'year' : 'years'}
                                            {getBestValue('duration') === p.id && ' ✓'}
                                        </td>
                                    ))}
                                </tr>
                                <tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id} className={getBestValue('rating') === p.id ? 'best' : ''}>
                                            ⭐ {p.rating}
                                            {getBestValue('rating') === p.id && ' ✓'}
                                        </td>
                                    ))}
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id} style={{ fontSize: '0.82rem', lineHeight: '1.5' }}>
                                            {p.description}
                                        </td>
                                    ))}
                                </tr>
                                    ...
                                    {selectedPolicyData.map(p => (
                                        <td key={p.id}>
                                            <ul style={{ paddingLeft: '16px', listStyle: 'disc' }}>
                                                {p.benefits.map((b, i) => (
                                                    <li key={i} style={{ fontSize: '0.8rem', marginBottom: '4px', color: 'var(--text-secondary)' }}>
                                                        {b}
                                                    </li>
                                                ))}
                                            </ul>
                    ...
                    <div style={{ textAlign: 'center', marginTop: '32px' }}>
                        <button className="btn btn-primary btn-lg" onClick={() => setViewMode('select')}>
                    ...
                    <aside className="glass-card filter-sidebar">
                        ...
                        <div className="filter-group">
                            ...
                            <div className="range-display">
                                <span>₹400</span>
                                <span><strong>{formatCurrency(filters.maxPremium)}</strong></span>
                            </div>
                        </div>
                        ...
                        <div className="filter-group">
                            ...
                            <input
                                ...
                                onChange={(e) => setFilters({ ...filters, minCoverage: Number(e.target.value) })}
                            />
                            <div className="range-display">
                                <span>₹0</span>
                                <span><strong>{formatCurrency(filters.minCoverage)}</strong></span>
                            </div>
                        </div>
                        ...
                        <div className="filter-group">
                            ...
                            <div className="range-display">
                                <span>1 yr</span>
                                <span><strong>{filters.maxDuration} yrs</strong></span>
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            onClick={() => setFilters({ type: 'All', maxPremium: 25000, minCoverage: 0, maxDuration: 30 })}
                        >
                            Reset Filters
                        </button>
                    </aside>
                    ...
                    <div>
                        ...
                        <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                            <span className="badge badge-teal" style={{ fontSize: '0.85rem', padding: '6px 14px' }}>
                                {selectedPolicies.length}/4 Selected
                            </span>
                            {selectedPolicies.length >= 2 && (
                                <button className="btn btn-primary btn-sm" onClick={() => setViewMode('compare')}>
                                    Compare Now →
                                </button>
                            )}
                            ...
                            {selectedPolicies.length >= 1 && selectedPolicies.length < 2 && (
                        ...
                        <div className="policy-grid">
                            {filteredPolicies.map(policy => (
                                <div
                                    key={policy.id}
                                    className={`glass-card policy-card ${selectedPolicies.includes(policy.id) ? 'selected' : ''}`}
                                    onClick={() => togglePolicy(policy.id)}
                                    style={{ opacity: selectedPolicies.length >= 4 && !selectedPolicies.includes(policy.id) ? 0.5 : 1 }}
                                >
                                    <div className="policy-card-header">
                                        <div>
                                            <h3>{policy.name}</h3>
                                            <span className={`badge ${typeColors[policy.type]} policy-type`} style={{ marginTop: '6px', display: 'inline-block' }}>
                                                {policy.type}
                                            </span>
                                        </div>
                                        <div className="policy-select-check">
                                    ...
                                    <div className="policy-details">
                                        <div className="policy-detail-item">
                                            ...
                                            <span className="value">{formatCurrency(policy.coverage)}</span>
                                        </div>
                                        <div className="policy-detail-item">
                                            <span className="label">Premium</span>
                                            <span className="value">{formatCurrency(policy.premium)}/yr</span>
                                        </div>
                                        <div className="policy-detail-item">
                                            <span className="label">Duration</span>
                                            <span className="value">{policy.duration} {policy.duration === 1 ? 'year' : 'years'}</span>
                                        </div>
                                        <div className="policy-detail-item">
                                            <span className="label">Rating</span>
                                            <span className="value">⭐ {policy.rating}</span>
                                        </div>
                                    </div>
                                    ...
                                    <ul className="policy-benefits">
                                        ...
                                        {policy.benefits.length > 3 && (
                                            <li style={{ color: 'var(--accent-teal)', fontWeight: 500 }}>
